...
import { TreasurySplitPayload, UserSerivce } from '@shared/services/user.service';
import { Subscription } from 'rxjs';

export type ClientPayClientDialogData = {
  client: Client;
...
@Component({
  selector: 'app-client-pay-client-dialog',
  templateUrl: './client-pay-client-dialog.component.html',
  styleUrls: ['./client-pay-client-dialog.component.scss'],
})
export class ClientPayClientDialogComponent implements OnInit, OnDestroy {
  form: FormGroup;
  saving = false;
  ...
  constructor(
    ...
    private ref: MatDialogRef<ClientPayClientDialogComponent, boolean>,
    @Inject(MAT_DIALOG_DATA) data: ClientPayClientDialogData
  ) {
  ...
  private buildTreasurySplitsPayload(): TreasurySplitPayload[] | null {
    ...
    if (this.treasuryOverAllocated()) {
      this.notify.push(this.translate.instant('tr_client_pay_client_over'), 'error');
      return null;
    }
  ...
  submit(): void {
    ...
    this.users
      ...
      .subscribe({
        next: () => {
          this.saving = false;
          this.notify.push(this.translate.instant('tr_client_pay_client_ok'), 'success');
          this.ref.close(true);
        },
        error: (err) => {
          this.saving = false;
          const msg = err?.error?.error || err?.error?.message || this.translate.instant('tr_unexpected_error_message');
