...
import { OrderPartyType, Product, ProductAcquiredFrom } from '@core/models/products.model';
...
export interface AddQuantityDialogData {
  product: Product;
}

type AddQtyTab = 'qty' | 'source' | 'payment';
...
export class AddQuantityDialogComponent implements OnInit, OnDestroy {
  activeTab: AddQtyTab = 'qty';
  form: FormGroup;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddQuantityDialogComponent, boolean>,
    @Inject(MAT_DIALOG_DATA) public data: AddQuantityDialogData,
  ...
  submit(): void {
    ...
    const branchId =
      this.product?.branch &&
      (typeof this.product.branch === 'object'
        ? String((this.product.branch as any)._id || '')
        : String(this.product.branch));
  ...
  private syncTreasuryOptions(): void {
    ...
    const deferredKey = AddQuantityDialogComponent.DEFERRED_KEY;
