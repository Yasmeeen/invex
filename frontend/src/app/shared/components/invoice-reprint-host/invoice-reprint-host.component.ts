...
/**
 * Isolated iframe print CSS — same visual weight as booking / shared invoice-container-print.
 * @page is 80mm × 297mm (portrait roll). Landscape on POS-80 garbles output.
 */
const INVOICE_RECEIPT_PRINT_CSS = `
  ...
  * {
  ...
  .invoice-item-attr { display: block; }
  .totals-table td,
...
<head>
  ...
  <style>${INVOICE_RECEIPT_PRINT_CSS}</style>
</head>
<body>${markup}</body>
</html>`);
    doc.close();
